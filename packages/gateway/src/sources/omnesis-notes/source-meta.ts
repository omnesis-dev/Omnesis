...
/**
 * Display identity (icon, label, colors) for the in-gateway omnesis-notes
 * source. Captured notes live in the `note_entries` ledger and are
 * projected into the document corpus by `upsert.ts`; this module only
 * deals with how the source surfaces in portal / iOS source lists.
 *
 * This source has no `defineSource()` descriptor, no auth flow, no sync
 * engine — it's gateway-internal. The display fields live in
 * `sync_state.icon/label/bg_color/accent_color` and are seeded directly
 * at boot rather than during a sync cycle.
 *
...
export const OMNESIS_NOTES_SOURCE_ID = "omnesis-notes";
export const OMNESIS_NOTES_PROVIDER_ID = "system";

export const OMNESIS_NOTES_LABEL = "Notes";
/** Brand blue — matches the Omnesis mark's primary fill. */
export const OMNESIS_NOTES_ACCENT_COLOR = "#1F6FEB";
export const OMNESIS_NOTES_BG_COLOR = "#0D1117";

/**
 * Resolves the absolute path of the brand PNG, anchored on this file's
 * compiled location. Both `src/sources/omnesis-notes/source-meta.ts` and
 * `dist/sources/omnesis-notes/source-meta.js` sit four directories below
 ...
 * start.
 */
export async function seedOmnesisNotesSourceMeta(writeGate: WriteGate): Promise<void> {
  const raw = await readBrandIconDataUri();
  const icon = raw ? ((await normalizeIcon(raw)) ?? undefined) : undefined;
  await writeGate.setSourceMeta(OMNESIS_NOTES_SOURCE_ID, {
    icon,
    label: OMNESIS_NOTES_LABEL,
    bgColor: OMNESIS_NOTES_BG_COLOR,
    accentColor: OMNESIS_NOTES_ACCENT_COLOR,
  });
}
