...
/**
 * Hot-path contract of the brief-claim invalidation subscriber: on
 ...
 * only for documents a live claim actually cites (the cheap read
 * guard) — and a failed invalidate is logged, never thrown back into
 * the emitter. Mirrors the temporal-annotation invalidator's contract.
 *
...
import { subscribeBriefClaimInvalidator } from "./brief-claim-invalidator.js";
...
function seedDoc(id: string): void {
  ...
  db.prepare(
    `INSERT INTO documents (id, provider_id, source_id, external_id, title, content, content_hash,
     ...
     VALUES (?, 'test-provider', 'test-source', ?, 'Deposit thread', 'we paid the venue deposit', 'hash-a', ?, ?, ?, ?)`,
...
function wire(over: Partial<Parameters<typeof subscribeBriefClaimInvalidator>[0]> = {}) {
  const bus = new EventBus();
  const invalidate = vi.fn((_docId: string, _now: number) => Promise.resolve(1));
      ...
      claims: [
        {
          ...
          evidenceDocId: "doc_cited",
          evidenceQuote: "we paid the venue deposit",
          claimBasis: "quoted",
          confidence: 0.8,
...
describe("brief-claim invalidator", () => {
  ...
  test("a document no live claim cites is guarded out before the writer hop", () => {
    const { bus, invalidate } = wire();
