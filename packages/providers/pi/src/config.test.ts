...
describe("the sessions-directory setting", () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "pi-config-"));
  });
  afterEach(() => {
    vi.unstubAllEnvs();
  ...
  test("a home-relative path is accepted, and the source is handed it resolved", () => {
    vi.stubEnv("HOME", dir);
    mkdirSync(join(dir, ".pi/agent/sessions"), { recursive: true });
    expect(issues("~/.pi/agent/sessions")).toEqual([]);

    const parsed = schema.parse({ sessionsPath: "~/.pi/agent/sessions" });
    ...
    expect(handed.sessionsPath).toBe(join(dir, ".pi/agent/sessions"));
  });
