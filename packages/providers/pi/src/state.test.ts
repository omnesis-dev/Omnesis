...
const SOURCE_ID = SourceId("pi:test");
const PROVIDER_ID = ProviderId("pi:test");
...
describe("piStateSpec via the host decorator", () => {
  ...
  async function makeVersionedInstance() {
    sessionsPath = mkdtempSync(join(tmpdir(), "pi-state-"));
    writeJsonl(join(sessionsPath, "pi-session-1.jsonl"), [
      { type: "session", id: "pi-session-1", cwd: "/work/example" },
      {
        id: "node-1",
        parentId: null,
        type: "message",
      ...
      },
      {
        ...
        type: "message",
        timestamp: iso(1),
        message: { role: "assistant", content: "Build is fine.", stopReason: "stop" },
      },
    ]);
    ...
    const instance = withVersionedState(raw, piStateSpec, {
      sourceId: "pi:test",
  ...
  test("resumes a cursor mid-cycle — not only the settled shape — and writes back an envelope", async () => {
    ...
    expect(piStateSpec.decode(midCycleCursor)).not.toBeNull();
