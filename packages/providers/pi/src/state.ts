 ...
 * re-lists Pi's session directory from disk on every cycle — it never trusts
 * the cursor to say which session files exist, only which ones can skip
 * re-parsing because they are unchanged. On a cycle that finishes a full pass
 * (`hasMore: false` with a safe scan), it reports every session's ids as
 * `presentExternalIds`, which is this source's corpus-wide "here is
 * everything upstream currently holds" signal — the same reconciliation
 * Obsidian's vault walk and the browser-history database read perform, not
 * the screen-time case's append-only watermark with nothing to reconcile
 * against. A stored cursor lost between cycles forces the next cycle to
 * re-parse every session file currently on disk instead of skipping the
 * unchanged ones, which is slower; it does not change what that cycle
 * concludes, because the conclusion comes from the filesystem, not the
 * cursor. Whatever session file Pi has already removed by the time of the
 * loss was equally unrecoverable with a healthy cursor — this source has no
 * access to a session file once Pi itself has removed it, cursor or no
 * cursor. `onUnreadable: "rebootstrap"` matches what a normal complete cycle
 * already does.
 *
...
export const piStateSpec: SourceStateSpec<LocalAgentSessionCursor> = {
  version: 1,
   ...
   * configured sessions directory; a complete scan rebuilds it from what is
   * on disk right now, so a file Pi has since removed drops out on the next
   * settled cycle rather than accumulating forever. The instance already
   * refuses to return a cursor over 16 MiB before this envelope wraps it
   * (`MAX_CURSOR_BYTES` in `@omnesis/source-sdk`'s `local-agent-sessions.ts`);
   * this mirrors that ceiling at the host layer, with headroom for the
   * envelope, so tripping it means the inner guard was bypassed or removed
   * rather than that a session archive grew.
   */
