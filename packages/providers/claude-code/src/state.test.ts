...
const SOURCE_ID = SourceId("claude-code:test");
const PROVIDER_ID = ProviderId("claude-code:test");
...
describe("claudeCodeStateSpec via the host decorator", () => {
  ...
  async function makeVersionedInstance() {
    sessionsPath = mkdtempSync(join(tmpdir(), "claude-code-state-"));
    writeJsonl(join(sessionsPath, "example-project", "claude-session-1.jsonl"), [
      {
        type: "user",
        uuid: "node-1",
        parentUuid: null,
    ...
    const instance = withVersionedState(raw, claudeCodeStateSpec, {
  ...
  test("resumes a cursor mid-cycle — not only the settled shape — and writes back an envelope", async () => {
    ...
    expect(claudeCodeStateSpec.decode(midCycleCursor)).not.toBeNull();
