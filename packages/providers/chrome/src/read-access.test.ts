...
test("fresh access checks reopen the declared input without parsing, copying or syncing", async () => {
  root = await mkdtemp(join(tmpdir(), "chrome-access-"));
  await mkdir(join(root, "Profile 1"));
  const input = join(root, "Profile 1", "Bookmarks");
  await writeFile(input, "not a valid source document or database");
  const instance = await definition.create!({
    accountId: "local",
    sourceId: SourceId("chrome-bookmarks:local"),
    providerId: ProviderId("chrome:local"),
    config: { basePath: root, profileDir: "Profile 1" },
  });
