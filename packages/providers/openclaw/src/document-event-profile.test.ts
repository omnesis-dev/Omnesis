...
const PROVIDER = ProviderId("openclaw");
const SOURCE = SourceId("openclaw:local");
...
/**
 * A conversation reached through a messaging platform. `identityFromSession`
 * in the OpenClaw reader yields exactly `{ channel, chatId }` and nothing
 * else, so the fixture carries no chat name or type — otherwise the pinning
 * below would pass on identity this harness never actually sends.
 */
...
function hostDoc(): DocumentInput {
  return render({ platform: "local", chatId: "main", chatName: undefined, chatType: undefined });
}
...
describe("openclaw documentEventProfile", () => {
  test("the declaration satisfies the source contract", () => {
    expect(() => validateDocumentEventProfile(profile, "source 'openclaw'")).not.toThrow();
  });
  ...
  test("the source is published under the source type the documents carry", () => {
    ...
    expect(source.id).toBe("openclaw");
  ...
  test("tags carry the harness and the channel, in that order", () => {
    expect(platformDoc().metadata.tags).toEqual(["openclaw", "slack"]);
    expect(hostDoc().metadata.tags).toEqual(["openclaw", "local"]);
    ...
    expect(declared?.canonicalValues).toContain("openclaw");
    expect(declared?.allowedValues).toBeUndefined();
  });

  test("the chat id is declared as identifying people", () => {
    // A direct-message id singles out the person on the other end, so the
    // operator must see the condition disclosed as one about a person.
    const declared = profile.metadataFields?.find((f) => f.path === "extra.chatId");
    expect(declared?.identifiesPeople).toBe(true);
  });

  test("the chat name and type are not declared, because this harness sends neither", () => {
    // They are part of the wire protocol but `identityFromSession` never
    // populates them, so every document carries null. A declared field that is
    // always absent compiles a predicate that can never match.
    const declared = new Set((profile.metadataFields ?? []).map((f) => f.path));
    expect(declared.has("extra.chatName")).toBe(false);
    expect(declared.has("extra.chatType")).toBe(false);
    expect(readPath(platformDoc(), "extra.chatName")).toBeNull();
    expect(readPath(platformDoc(), "extra.chatType")).toBeNull();
  });

  test("a host session uses the main chat id", () => {
    expect(readPath(hostDoc(), "extra.chatId")).toBe("main");
  });
  ...
  test("the constant-valued fields the renderer writes are deliberately not declared", () => {
    ...
    const doc = platformDoc();
    expect(readPath(doc, "extra.harness")).toBe("openclaw");
