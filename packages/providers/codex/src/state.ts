 ...
 * re-lists Codex's session directories from disk on every cycle — it never
 * trusts the cursor to say which files exist, only which ones can skip
 ...
 * cursor. Whatever Codex has already rotated out of `~/.codex/sessions` and
 * `archived_sessions` by the time of the loss was equally unrecoverable with
 * a healthy cursor — this source has no access to a session file once Codex
 * itself has removed it, cursor or no cursor. `onUnreadable: "rebootstrap"`
...
export const codexStateSpec: SourceStateSpec<LocalAgentSessionCursor> = {
  version: 1,
   ...
   * configured roots; a complete scan rebuilds it from what is on disk right
   * now, so a file Codex has since removed drops out on the next settled
   * cycle rather than accumulating forever. The instance already refuses to
   * return a cursor over 16 MiB before this envelope wraps it (`MAX_CURSOR_BYTES`
   * in `@omnesis/source-sdk`'s `local-agent-sessions.ts`); this mirrors that
   * ceiling at the host layer, with headroom for the envelope, so tripping it
   * means the inner guard was bypassed or removed rather than that a session
   * archive grew.
   */
