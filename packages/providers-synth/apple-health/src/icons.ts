...
/**
 * Apple's brand guidelines explicitly forbid third-party use of the
 * first-party Health app icon in other software UIs. The synth source
 * therefore owns its own glyph — a Lucide SVG (ISC-licensed) tinted to
 * Apple Health's accent color, embedded as a data URI so the portal's
 * strict CSP (`img-src 'self' data: blob:`) lets it through.
 */
...
export const appleHealthIconDataUri = svgDataUri(tintLucideSvg(HEART_PULSE_SVG, "#FF2D55"));
